<?php                       //For storing the data coming from the client side to the database
    ...
    if(isset($_POST['pid'])){                    //if any pid is posted from the ajax request then we will get by using this method
        ...
        if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true)
        {
            ...
            if(!isset($row['product_code'])){
                ...
                echo 'alert("Item added to your cart!");'; 
                echo '</script>';
            }
            else {
                echo '<script type="text/javascript">'; 
                echo 'alert("Item already added to your cart!");'; 
                echo '</script>';
            }
        }
        else {
    ...
    if(isset($_POST['action']) && isset($_POST['action']) == 'order'){
        ...
        $stmt = $conn->prepare("INSERT INTO orders (name,email,mobile,address,pmode,products,grand_total) VALUES (?,?,?,?,?,?,?)");
        $stmt->bind_param("sssssss",$name,$email,$mobile,$address,$pmode,$products,$grand_total);
        $stmt->execute();
        $data = '<div>
